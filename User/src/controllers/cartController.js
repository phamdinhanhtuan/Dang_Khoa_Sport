...
exports.addToCart = catchAsync(async (req, res, next) => {
    ...
    if (product.stock < qty) {
        return next(new AppError(`Rất tiếc, chỉ còn ${product.stock} sản phẩm trong kho`, 400));
    }
    ...
    if (req.user) {
        ...
        if (idx > -1) {
            const newTotal = cart.items[idx].quantity + qty;
            if (newTotal > product.stock) {
                return next(new AppError(`Bạn đã có ${cart.items[idx].quantity} sản phẩm này trong giỏ. Kho chỉ còn ${product.stock}.`, 400));
            }
    ...
    if (req.user) {
        ...
        if (cart) {
            ...
            if (item) {
                // Validate Stock
                if (qty > 0) {
                    // We need to fetch product to check stock. 
                    // item.product might be populated or just ID depending on how we fetched cart.
                    // In getCartHelper we populated, but here we did findOne without populate?
                    // Let's assume item.product is ID or check it.
