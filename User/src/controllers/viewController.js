...
exports.getShop = catchAsync(async (req, res) => {
    ...
    const limit = req.query.limit * 1 || 12; // Force 12 per page

    // Enforce limits in service call
    const queryObj = { ...req.query, page, limit };

    const { products, total } = await productService.getAllProducts(queryObj);
    const categories = await Category.find();

    // We must pass the calculated values to the view
